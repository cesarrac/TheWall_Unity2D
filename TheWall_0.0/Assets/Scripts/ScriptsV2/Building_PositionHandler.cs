...
public class Building_PositionHandler : MonoBehaviour {
	...
	public int currBuildingCost; // this will charge the resources manager with the cost sent from UI handler
	...
	void FollowMouse(){
		...
		if (tileType == TileData.Types.sextractor || tileType == TileData.Types.mextractor || tileType == TileData.Types.lextractor) {
		...
		if (resourceManager.ore >= currBuildingCost) {
			canAfford = true;
		}
		...
		if (Input.GetMouseButtonDown (0) && canBuild && canAfford) {			// So LEFT CLICK to BUILD!!
			// Subtract the cost
			resourceManager.ChangeResource("Ore", -currBuildingCost);
			mapPosX = mX;
			mapPosY = mY;
			// stop following and tell grid to swap this tile to this new building
			resourceGrid.SwapTileType (mX, mY, tileType);
			followMouse = false;
			Destroy (gameObject); // Destoy this because resourceGrid just discovered it for us
		}
	}
